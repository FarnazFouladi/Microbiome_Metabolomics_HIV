...
source("code/load.R")
# Load data
expvec <- read_rds("data/external_dataset/Fulcher_2022/JAMSBETA/PRJNA836336_Expvec_list_of_SummarizedExperiment_objects.rds")
# Taxonomies
...
saveRDS(gut_taxonomies, file = "data/external_dataset/Fulcher_2022/JAMSBETA/taxonomies.rds")
...
genome_com <- genome_com[, colnames(totalbases)]

# Extract feature tables
features <- names(expvec)
features_tables <- lapply(features, function(x) {
  df_tmp <- as.data.frame(assays(expvec[[x]])) %>%
    filter(group_name == "BaseCounts") %>%
    dplyr::select(-c("group", "group_name"))
  df_tmp <- df_tmp[, colnames(totalbases)]
})
...
features_tables_ppm <- lapply(features, function(x) {
  df_tmp <- as.data.frame(assays(expvec[[x]])) %>%
    ...
    dplyr::select(-c("group", "group_name"))
  df_tmp <- df_tmp[, colnames(totalbases)]
  stopifnot(all.equal(colnames(df_tmp), colnames(totalbases)))
...
# Metadata
meta <- read_tsv("data/external_dataset/Fulcher_2022/UCLA_metadata.tsv") # 82 samples
meta <- meta %>% filter(Sample %in% colnames(totalbases))
meta <- meta[match(colnames(totalbases), meta$Sample), ]

stopifnot(all.equal(meta$Sample, colnames(features_filtered_tables[[1]])))

out_dir <- file.path("data/external_dataset/Fulcher_2022", "processed_data")
...
for (f in c("LKT", "GO")) {
  ...
  rownames(ann_tmp) <- gsub("\\(|\\)", ".", rownames(ann_tmp))
  rownames(ann_tmp) <- gsub("GO:", "GO.", rownames(ann_tmp))
  ...
  saveRDS(phy_taxa, file.path(out_dir, paste0(f, "_phy.rds")))
}
