...
phy_obj_go <- readRDS("data/processed_data/oral/GO_phy.rds")
ann_go <- microbiome::tax_tibble(phy_obj_go) %>% as.data.frame() %>%
...
mtb_ann <- mtb_ann %>% dplyr::rename(metabolite_ID = accession, Metabolites = Compound_name) %>%
  dplyr::select(-c("name"))


######################### species
sub_dir <- file.path(output_dir, "Oral_Species")
name <- "species"
res_nc_oral_species <- readRDS(file.path(sub_dir, paste0(name, "_g1.rds")))
res_sc_oral_species <- readRDS(file.path(sub_dir, paste0(name, "_g2.rds")))
phylum_levels <- c(
...
res_oral_species <- read.csv(file.path(sub_dir, paste0(name, "_differential_correlations.csv")), check.names = F)
sig_diff_oral_species <- res_oral_species[res_oral_species$diff_cor > threshold & res_oral_species$p < pval_t, ]
sig_diff_save <- sig_diff_oral_species %>%
...
create_networks(
  g1_mat = res_nc_oral_species$corr, g1_mat_cooccur = res_nc_oral_species$mat_cooccur,
  g2_mat = res_sc_oral_species$corr, g2_mat_cooccur = res_sc_oral_species$mat_cooccur,
  sig_diff = sig_diff_oral_species,
  ...
  cexNodes = 1.5, cexNodes2 = 0.5,
  cexHubs = 2, cexHubs2 = 0.8,
...
sub_dir <- file.path(output_dir, "Oral_GO")
name <- "go"
res_nc_oral_go <- readRDS(file.path(sub_dir, paste0(name, "_g1.rds")))
res_sc_oral_go <- readRDS(file.path(sub_dir, paste0(name, "_g2.rds")))
res_oral_go <- read.csv(file.path(sub_dir, paste0(name, "_differential_correlations.csv")), check.names = F)
sig_diff_oral_go <- res_oral_go[res_oral_go$diff_cor > threshold & res_oral_go$p < pval_t, ]
sig_diff_save <- sig_diff_oral_go %>%
  mutate(Taxa = gsub("data1 - ","", Taxa),
...
create_networks(
  g1_mat = res_nc_oral_go$corr, g1_mat_cooccur = res_nc_oral_go$mat_cooccur,
  g2_mat = res_sc_oral_go$corr, g2_mat_cooccur = res_sc_oral_go$mat_cooccur,
  sig_diff = sig_diff_oral_go,
  taxonomies = NULL,
...
######################### Oral species and Oral MTB Pos
sub_dir <- file.path(output_dir, "Oral_MTB_Pos")
name <- "mtb_pos"
res_nc_oral_mtb_pos <- readRDS(file.path(sub_dir, paste0(name, "_g1.rds")))
res_sc_oral_mtb_pos <- readRDS(file.path(sub_dir, paste0(name, "_g2.rds")))
res_oral_mtb_pos <- read.csv(file.path(sub_dir, paste0(name, "_differential_correlations.csv")), check.names = F)
sig_diff_oral_mtb_pos <- res_oral_mtb_pos[res_oral_mtb_pos$diff_cor > threshold & res_oral_mtb_pos$p < pval_t, ]
sig_diff_save <- sig_diff_oral_mtb_pos %>%
  mutate(Taxa = gsub("data1 - ","", Taxa),
         name = gsub("data2 - ","", name)) %>%
...
create_networks(
  g1_mat = res_nc_oral_mtb_pos$corr, g1_mat_cooccur = res_nc_oral_mtb_pos$mat_cooccur,
  g2_mat = res_sc_oral_mtb_pos$corr, g2_mat_cooccur = res_sc_oral_mtb_pos$mat_cooccur,
  sig_diff = sig_diff_oral_mtb_pos,
  taxonomies = NULL,
...
######################### Oral species and Oral MTB Neg
sub_dir <- file.path(output_dir, "Oral_MTB_Neg")
name <- "mtb_neg"
res_nc_oral_mtb_neg <- readRDS(file.path(sub_dir, paste0(name, "_g1.rds")))
res_sc_oral_mtb_neg <- readRDS(file.path(sub_dir, paste0(name, "_g2.rds")))
res_oral_mtb_neg <- read.csv(file.path(sub_dir, paste0(name, "_differential_correlations.csv")), check.names = F)
sig_diff_oral_mtb_neg <- res_oral_mtb_neg[res_oral_mtb_neg$diff_cor > threshold & res_oral_mtb_neg$p < pval_t, ]
sig_diff_save <- sig_diff_oral_mtb_neg %>%
  mutate(Taxa = gsub("data1 - ","", Taxa),
         name = gsub("data2 - ","", name)) %>%
  dplyr::rename(metabolite_ID = name) %>%
  left_join(mtb_ann) %>%
...
create_networks(
  g1_mat = res_nc_oral_mtb_neg$corr, g1_mat_cooccur = res_nc_oral_mtb_neg$mat_cooccur,
  g2_mat = res_sc_oral_mtb_neg$corr, g2_mat_cooccur = res_sc_oral_mtb_neg$mat_cooccur,
  sig_diff = sig_diff_oral_mtb_neg,
  taxonomies = NULL,
  t = NULL, datasets = c("Oral Species", "Oral MTB-Neg"),
  threshold = threshold, pval_th = pval_t,
  layoutGroup = "union", layoutGroup2 = "union",
  repulsion = 0.85, repulsion2 = 0.7,
  cexNodes = 1, cexNodes2 = 0.5,
...
######################### Oral species and Plasma MTB Pos
...
create_networks(
  ...
  taxonomies = NULL,
  t = NULL, datasets = c("Oral Species", "Plasma MTB-Pos"),
  threshold = threshold, pval_th = pval_t,
  layoutGroup = "union", layoutGroup2 = "union",
  repulsion = 0.85, repulsion2 = 0.7,
  cexNodes = 1, cexNodes2 = 0.5,
...
######################### Oral species and Plasma MTB Neg
