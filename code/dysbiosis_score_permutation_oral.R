...
species2 <- dysbiosis_score(
  ...
  assay_name, tax_level = c("Species", "Species"), prv_cut, method, cov_cat, cov_num, threshold, pval_t, name = "species2", two_datasets = TRUE, output_dir = file.path(output_dir, paste0(ifelse(sample_type == "Gut", "Oral", "Gut"), "_Species")), perm = i
)
...
# Oral species and oral metabolites positive channels
...
# Oral species and oral metabolites negative channels
...
# Oral species and plasma metabolites positive channels
...
# Oral species and plasma metabolites negative channels
...
# Number of differential correlations for each species
