...
scfa_long$scfa <- stringr::str_to_sentence(scfa_long$scfa)
scfa_long$scfa <- gsub("_"," ",scfa_long$scfa)

all_scfa_names <- stringr::str_to_sentence(all_scfa_names)
all_scfa_names <- gsub("_"," ",all_scfa_names)
...
for (s in all_scfa_names) {
  ...
  fit <- lm_test(scf_sub, x = "status", y = "value", plot_title = s, y_label = "Log transformed")
...
plots <- lapply(unique(scf_sub$env), function(x) {
  ...
  df_p_sig1 <- df_p_sig %>% filter(grepl(x, SCFA))
