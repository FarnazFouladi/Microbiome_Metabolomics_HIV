...
macs_species <- read.csv("Outputs/Differential_Correlations/Tables/gut_species_disco.csv", check.names = F)
macs_go <- read.csv("Outputs/Differential_Correlations/Tables/gut_go_disco.csv", check.names = F)
...
pdf(file.path("Outputs/External_Dataset", "Genus_disco_heatmap.pdf"), 30, 60)
print(h)
dev.off()
