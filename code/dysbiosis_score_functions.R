...
dysbiosis_score <- function(data_g1 = data_list_nc_tse[c(1, 3)],
                            ...
                            assay_name, tax_level = c(NULL, NULL), prv_cut, method, cov_cat, cov_num, threshold = 0.3, pval_t, name = "gut_mtb_pos", two_datasets = TRUE, output_dir) {
  # Create directory
  ...
  # Save the Secom outputs
  saveRDS(res_secom_g1, file.path(output_dir, paste0(name, "_g1.rds")))
  saveRDS(res_secom_g2, file.path(output_dir, paste0(name, "_g2.rds")))
  ...
  write.csv(res, file.path(output_dir, paste0(name, "_differential_correlations.csv")), row.names = F, quote = F)
  ...
  if (two_datasets) {
    ...
    # Number of correlations that are higher in g2
    res_sc_p <- res %>%
      filter(p < pval_t & diff_cor > threshold, !is.na(p)) %>%
      filter(sign == paste0("g2", "+") | sign == paste0("g2", "-")) %>%
      group_by(Taxa) %>%
      summarise(cor2 = n()) %>%
      mutate(Taxa = gsub("data1 - |data2 - ", "", Taxa))

    # Number of correlations that are higher in g1
    res_nc_p <- res %>%
      filter(p < pval_t & diff_cor > threshold, !is.na(p)) %>%
      filter(sign == paste0("g1", "+") | sign == paste0("g1", "-")) %>%
      group_by(Taxa) %>%
      summarise(cor1 = n()) %>%
      mutate(Taxa = gsub("data1 - |data2 - ", "", Taxa))
...
fisher_test <- function(mat1 = g1_mat, mat2 = g2_mat, mat1_cooccur = g1_mat_cooccur, mat2_cooccur = g2_mat_cooccur) {
  ...
  res <- lapply(1:nrow(df), function(i) {
    ...
    df_tmp <- data.frame(
      ...
      sign = ifelse(abs(df$cor_1[i]) > abs(df$cor_2[i]) & df$cor_1[i] > 0, "g1+", ifelse(
        abs(df$cor_1[i]) > abs(df$cor_2[i]) & df$cor_1[i] < 0, "g1-", ifelse(
          abs(df$cor_1[i]) < abs(df$cor_2[i]) & df$cor_2[i] > 0, "g2+", "g2-"
        )
      ))
    )
  })
